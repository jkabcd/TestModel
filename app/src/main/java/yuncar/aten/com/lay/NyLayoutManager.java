...
public class NyLayoutManager extends RecyclerView.LayoutManager {
        ...
        for (int i = 0; i < getItemCount(); i++) {
            if (Rect.intersects(displayFrame, itemFrames.get(i))) {//判断是否在屏幕中
                ...
                layoutDecorated(view, rect.left - horizontalOffset, rect.top - verticalOffset,
                        rect.right - horizontalOffset, rect.bottom - verticalOffset);//将ViewLayout出来，显示在屏幕上，内部会自动追加上该View的ItemDecoration和Margin。此时我们的View已经可见了
            }
        }
    ...
    public int scrollHorizontallyBy(int dx, RecyclerView.Recycler recycler, RecyclerView.State state) {
        Log.e(TAG,"horizontalOffset测试："+horizontalOffset);
        detachAndScrapAttachedViews(recycler);
...
//        }
        ...
        fill(recycler, state);
        horizontalOffset += dx;
        Log.e(TAG,"dx="+dx+"&&&&&&&&"+"horizontalOffset="+horizontalOffset);
        return dx;
    }
