...
//得到当前RecyclerView的布局管理器
        ...
        for (int m = 0, size = parent.getChildCount(); m < size; m++) {
            ...
            if (mStickyView.isStickyView(view)) {
                //当前UI当中是否找到了需要吸附的View，此时mCurrentUIFindStickView设置为true
                mCurrentUIFindStickView = true;
                getStickyViewHolder(parent);//这个方法是得到吸附View的viewHolder
                cacheStickyViewPosition(m);//缓存需要吸附的View在列表当中的下标position

                if (view.getTop() <= 0) { //如果当前吸附的view距离 顶部小于等于0，然后给吸附的View绑定数据，计算View的宽高
                    bindDataForStickyView(mLayoutManager.findFirstVisibleItemPosition(), parent.getMeasuredWidth());
                } else { //如果大于0，从position缓存中取得当前的position，然后绑定数据，计算View的宽高
                ...
                if (view.getTop() > 0 && view.getTop() <= mStickyItemViewHeight) {
                    mStickyItemViewMarginTop = mStickyItemViewHeight - view.getTop();//计算吸附的View距离顶部的高度
                } else {
                ...
                drawStickyItemView(c);//绘制吸附的View
                break;
            }
        }

        if (!mCurrentUIFindStickView) {//如果在当前的列表视图中没有找到需要吸附的View
            mStickyItemViewMarginTop = 0;
